...
def process_voucher_eligibility(
    ...
    # Use first non-blank value per (ALU, column) to handle duplicate ALU rows
    # correctly instead of blindly taking the first row via drop_duplicates.
    ecom_df[style_col] = ecom_df[style_col].astype(str).str.strip()

    def _build_ecom_lookup(df, key_col):
        """
        Build a lookup dict: {alu: {col: first_non_blank_value}}.
        When an ALU appears more than once (e.g. multiple colour variants),
        drop_duplicates would silently discard all but the first row — which
        may have blank Ecom Status, a future launch date, etc.
        Instead, for each column we take the first non-blank value across all
        rows sharing the same ALU.
        """
        def _is_blank(v):
            if v is None:
                return True
        ...
        for _, row in df.iterrows():
            ...
            for col_name, val in row.items():
                if col_name == key_col:
                    continue
                # Only overwrite if we don't yet have a non-blank value
    ...
    def evaluate_sku_for_voucher(row, keywords, vc_pct):
        ...
        # Filter 4: SRP (0 or blank is allowed; 1–16 inclusive is excluded)
        srp_raw = row.get("SRP", "")
        srp = _to_float(srp_raw)
        # Treat blank/null SRP as 0 (no sale price = full price, allowed)
        if srp is None:
            srp = 0.0
        if srp != 0 and srp <= 16:
            return ""
